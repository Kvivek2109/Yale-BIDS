...
const processBackgroundJob = (task_id, count, id_list) => {
    ...
    setTimeout(() => {
        ...
        backgroundJobs[task_id].run_seconds = (new Date() - new Date(backgroundJobs[task_id].created_time)) / 1000;
    }, 5000);
};
...
router.get('/', async function (req, res, next) {
    ...
    if (term) {
        try {
            const response = await axios.get(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&usehistory=y&retmode=json&retstart=0&retmax=10000&term=${term}`);
